...
| Conversation planning interviews incomplete work, then drafts a collision-safe marked file after authorization | `to-plan-direct`, `to-plan-authorized-draft-direct`, and `to-plan-prior-confirmed-novel` | The forced `to-plan-authorized-draft-direct` countercase passed its artifact validator, forced-read integrity check, and all five rubric criteria in one audited run; other cases and repetitions remain pending. |
...
| A target-runtime assumption can invalidate the implementation approach, so a bounded proof gates dependent work; evidence-backed routine work remains one slice without a proof task | `to-plan-material-assumption-proof-calibration` uses the synthetic report-publisher and mount-config fixture, requires a T2-to-T1 edge, and checks the graph is acyclic; `to-plan-authorized-draft-direct` is the no-proof counterexample | Both selected forced cases passed their artifact validators, forced-read integrity checks, objective grading, and all five rubric criteria in one audited repetition each. Accepted run IDs, costs, fingerprints, and digests are in [`2026-09-23-to-plan-proof-case-focused-run.md`](2026-09-23-to-plan-proof-case-focused-run.md). This is targeted non-gating evidence; the synthetic fixture cannot establish actual target-runtime mount topology. |

...
| A target-runtime assumption can invalidate the implementation approach, so a bounded proof gates dependent work; evidence-backed routine work remains one slice without a proof task | `to-plan-material-assumption-proof-calibration` requires a T2-to-T1 edge and acyclic graph; `to-plan-authorized-draft-direct` is the no-proof counterexample | Corpus and artifact validators pass; final focused model run passed both cases on the forced arm (Terra medium, Sol high, one repetition). The first run exposed an overbroad concision check in the countercase rubric; it was narrowed to the approved one-slice/no-proof distinction before the final run. |

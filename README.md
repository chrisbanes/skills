...
- [`implement-with-subagents`](skills/implement-with-subagents/SKILL.md) — implement or review supplied-task orchestration through runtime-specific implementation owners, preserving atomic work, task-scoped acceptance, and repair ownership; maps Codex, Claude Code, OpenCode, and extension-based Pi delegation. Implementation mode requires Matt Pocock's external `implement` skill.
- [`to-plan`](skills/to-plan/SKILL.md) — turn one ready GitHub issue or an in-chat task into a repository-grounded, executor-ready implementation recipe with concrete tests and bounded repair rules.
